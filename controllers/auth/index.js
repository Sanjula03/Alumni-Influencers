// auth controller - register, login, logout, email verification, password reset
'use strict'
...
// custom routes since auth doesnt follow standard CRUD
exports.setup = function(app) {

  app.post('/auth/register', authLimiter, registerValidation, async function(req, res) {
    try {
  ...
  app.post('/auth/login', authLimiter, loginValidation, async function(req, res) {
    try {
      var errors = validationResult(req);
      if (!errors.isEmpty()) {
        return res.status(400).json({ success: false, errors: errors.array() });
